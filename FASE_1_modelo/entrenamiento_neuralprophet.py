# CONVERSIÓN DE entrenamiento_prophet a este utilizando Gemini. Cambiaré para cuando se suba a github
...
# Cambiamos Prophet por NeuralProphet
from neuralprophet import NeuralProphet, set_log_level

warnings.filterwarnings("ignore", category=FutureWarning)

# Desactivar logs excesivos de NeuralProphet (opcional pero recomendado)
set_log_level("ERROR")
...
# Lista con todas las promociones. CONSULTAR A ROSA ANA porque hay cosas que Patricia puso que ella no
promociones = [
    # ENERO 2023
...
regresores_validos = []
for reg in regresores:
    # Si la columna tiene más de 1 valor distinto (ej. 0 y 1), es útil
    if df_final[reg].nunique() > 1:
        model.add_future_regressor(name=reg)
        regresores_validos.append(reg)
    else:
        print("Es constante")
...
columnas_necesarias = ['ds', 'y'] + regresores_validos
df_final = df_final[columnas_necesarias]


df_train, df_test = model.split_df(df_final, valid_p=0.15)

metrics = model.fit(df_train, validation_df=df_test, freq="D", learning_rate=0.03)

# ==========================================
# PREDICCIÓN A FUTURO (180 días)
# ==========================================

# Para NP, primero construimos un DataFrame con las fechas futuras y sus regresores
fechas_futuras = pd.date_range(start=df_final['ds'].max() + pd.Timedelta(days=1), periods=180)
df_futuro_regresores = pd.DataFrame({'ds': fechas_futuras})

# Le pegamos el calendario
df_futuro_regresores = pd.merge(df_futuro_regresores, df_calendario, on='ds', how='left')

# Le pegamos los Tiers
df_futuro_regresores['promo_tier_1'] = df_futuro_regresores['ds'].isin(fechas_t1).astype(int)
df_futuro_regresores['promo_tier_2'] = df_futuro_regresores['ds'].isin(fechas_t2).astype(int)
df_futuro_regresores['promo_tier_3'] = df_futuro_regresores['ds'].isin(fechas_t3).astype(int)

# Limpiamos los regresores futuros
for reg in regresores:
    df_futuro_regresores = limpiar_regresor(df_futuro_regresores, reg)

columnas_futuras = ['ds'] + regresores_validos
df_futuro_regresores = df_futuro_regresores[columnas_futuras]

# Usamos make_future_dataframe integrando los datos históricos y los regresores futuros
future = model.make_future_dataframe(
    df=df_final,
    regressors_df=df_futuro_regresores,
    periods=180
)
...
# NP genera columnas "yhat1" (predicción central), "yhat1 25.0%" y "yhat1 75.0%" (intervalos)
# Filtramos solo el futuro para ver el trimestre
trimestre = forecast[forecast["ds"] > df_final["ds"].max()][
    ["ds", "yhat1", "yhat1 25.0%", "yhat1 75.0%"]
].reset_index(drop=True)

# Renombramos para mantener tu formato original
trimestre = trimestre.rename(columns={
    "yhat1": "yhat", 
    "yhat1 25.0%": "yhat_lower", 
    "yhat1 75.0%": "yhat_upper"
})

print("---- PREDICCIÓN PRÓXIMOS DÍAS ----")
print(trimestre.head(30))

# Imprimimos las métricas de error (NeuralProphet te las da directamente en el objeto metrics devuelto por fit)
print("\n---- MÉTRICAS DEL MODELO (TRAIN vs TEST) ----")
# Seleccionamos el último epoch para ver el error final
metricas_finales = metrics.tail(1)[["MAE", "RMSE", "MAE_val", "RMSE_val"]]
print(metricas_finales)

# Gráficos
fig1 = model.plot(forecast)
plt.title("Previsión de pedidos omnicanal (NeuralProphet)")
plt.xlabel("Fecha")
